...
from features.features import filter_with_matches
import pgm.tracks as tracks
...
class LoopClosure:
    def __init__(self, pkl_path):
        # Folders
        self.stage3_pkl_path = sys_utils.path_to_current_os(pkl_path)
        self.stage3_dir, _, _ = sys_utils.dir_name_ext(self.stage3_pkl_path)
        self.out_path = os.path.dirname(self.stage3_dir)
        self.stage5_dir = os.path.join(self.out_path, 'stage5')
        sys_utils.clear_and_make_dir(self.stage5_dir)
        self.num_of_lc = 0
        self.lc_msg = 'lc_0'
        # init kitti
        ...
        self.s3_ext_li_to_l0_s = utils.geometry.concat_cj_to_ci_s(s3_ext_lj_to_li_s)
        self.s3_dws = utils.geometry.get_dws_from_cam_to_world_s(self.s3_ext_li_to_l0_s)
        self.marg_covs = []
    ...
    def check_add_edges(self, i, n, prev_Pose3_ln_to_li, prev_cov_ln_cond_li, do_lc=True): # e.g. 57, 132
        ...
        kp_li, kp_ri, pc_lr_i_in_li, kp_ln, kp_rn, pc_lr_n_in_ln, matches_li_ln = features2d.match_two_pairs(i_kf, n_kf)
        kp_li_m, kp_ri_m, pc_n_m = filter_with_matches(matches_li_ln, [kp_li, kp_ri], [pc_lr_n_in_ln])
        pnp_ext_ln_to_li, pnp_inliers_bool, _, _ = pnp.pnp_stereo_ransac(kp_li_m, kp_ri_m, pc_n_m, self.k, self.ext_l_to_r, max_iters=15)
        ...
        if pnp_perc_inliers >= INLIERS_PERC_LC_THRESH and do_lc: # add edge
            ...
            tracks_db = tracks.StereoTracksDB()
        ...
        return added_edge, pnp_perc_inliers
    ...
    def main(self, do_lc=True):
        ...
        while n < len(self.keyframes_idx): # n in [1,2,...,276]
            ...
            for i in range(n-30):
                ...
                Pose3_ln_to_li, cov_ln_cond_li, simp_path = utils.shortest_path.Pose3_and_cov_ln_to_li_from_pred(i, n, self.from_to_Pose3_dict, self.cov_ln_cond_li_dict, pred_to_n[n])
                ...
                if mahal_dist < MAHAL_THRESH and n-i > 20:
                    entered_pnp = True
                    added_edge, inliers_perc = self.check_add_edges(i, n, Pose3_ln_to_li, cov_ln_cond_li, do_lc=do_lc)
                    if added_edge: 
            ...
            if lc_edges_idx and do_lc:
                ...
                sys_utils.make_dir_if_needed(self.cur_lc_dir)
                # update table
    ...
    def compute_ext_cov_li_to_l0_s_from_dijk(self, title):
        ...
        for n in range(1, num_frames): # [1,...,276]
            # find ln_to_l0_s
            Pose3_ln_to_l0, cov_ln_cond_l0, simp_path = utils.shortest_path.Pose3_and_cov_ln_to_li_from_pred(0, n, self.from_to_Pose3_dict, self.cov_ln_cond_li_dict,
                                                                                                       pred_to_n[n])
        ...
        dijk_dws = utils.geometry.get_dws_from_cam_to_world_s(dijk_ext_li_to_l0_s)
        dijk_sd = (dijk_dws, f'dijk_dws_{self.num_of_lc}', 'rgb(255,7,137)')
        utils.plot.plotly_scatter(x=self.keyframes_idx, y=dijk_det_li_on_l0_s, yaxis='det of cov', plot_dir=self.cur_lc_dir,
                            title=f"dijk_det_cov_li_cond_on_l0_s_{title}", save=True, plot=False)
        utils.plot.plotly_scatter(x=self.keyframes_idx, y=num_steps_to_zero, yaxis='num of steps to zero', plot_dir=self.cur_lc_dir,
                            title=f"dijk_length_path_to_zero_{title}", save=True, plot=False)
        tmp_sd = self.sd_list + [dijk_sd]
        utils.plot.plotly_2D_cams(tmp_sd, title=f"dijk_{title}", plot_dir=self.cur_lc_dir, frames_idx=self.keyframes_idx, save=True, plot=False)
        utils.plot.plotly_3D_cams(tmp_sd, title=f"dijk_{title}", plot_dir=self.cur_lc_dir, frames_idx=self.keyframes_idx, save=True, plot=False)
    ...
    def output_results(self):
        ...
        new_dws = utils.geometry.get_dws_from_cam_to_world_s(ext_li_to_l0_s)
        new_sd = (new_dws, f'after LC {self.num_of_lc}', utils.plot.get_color(self.num_of_lc))
        ...
        sys_utils.clear_and_make_dir(new_stage5_dir)
    ...
    def output_after_main(self):
        ...
        if self.errors_before and self.errors_after:
                ...
                utils.plot.plotly_scatters(err_dict, x=x, title="stage5_pose_graph_errors", plot_dir=self.stage5_dir, yaxis="error", xaxis="LC num", save=True, plot=False)
