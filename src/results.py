...
from utils import utils_sys, utils_img
import kitti
...
def output_results(out_path, ext_li_to_l0_s_dict, frames_idx,  title, start_time, plot=False, save=True):
    ...
    utils_sys.make_dir_if_needed(res_dir)
    stats = [f'** {title.upper()} **']
    # kitti
    ext_li_to_l0_s_kitti = kitti.read_poses_cam_to_world(frames_idx)
    rj_to_ri_s_kitti, tj_to_ti_s_kitti = utils_img.rot_trans_j_to_i_s(ext_li_to_l0_s_kitti)
    if startframe != 0: ext_li_to_l0_s_kitti = utils_img.make_relative_to_ci(ext_li_to_l0_s_kitti)
    # compute relative rotation translation diffs
    rot_diffs_rel_mult, trans_diffs_rel_mult = utils_img.get_rot_trans_diffs_from_mats(ext_li_to_l0_s_kitti, *ext_li_to_l0_s_mult)
    # normalize by number of frames
    sizes = np.array(frames_idx)[1:] - np.array(frames_idx)[:-1]
    # output relative rots_trans stats and plots
    rot_trans_stats_rel, rots_total_error_rel, trans_total_error_rel = utils_img.rot_trans_stats(rot_diffs_relative, trans_diffs_relative, frames_idx, rel_or_abs="relative")
    ...
    rot_diffs_abs, trans_diffs_abs = utils_img.get_rot_trans_diffs_from_mats(ext_li_to_l0_s[1:], ext_li_to_l0_s_kitti[1:])
    # output absolute rots_trans stats and plots
    rot_trans_stats_abs, rots_total_error_abs, trans_total_error_abs = utils_img.rot_trans_stats(rot_diffs_abs, trans_diffs_abs, frames_idx, rel_or_abs="absolute")
    ...
    my_dws = utils_img.get_dws_from_cam_to_world_s(ext_li_to_l0_s)
    kitti_dws = utils_img.get_dws_from_cam_to_world_s(ext_li_to_l0_s_kitti)
